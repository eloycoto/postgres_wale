...
su - postgres -c "envdir /etc/wal-e.d/env /usr/local/bin/wal-e backup-fetch /opt/database LATEST" || exit 1
sudo -u postgres rm $DB_PATH/backup_label
sudo -u postgres $PG_BIN/pg_resetxlog -f $DB_PATH
echo 1 > /wale_finished
sudo -u postgres /usr/lib/postgresql/9.6/bin/postgres -D /opt/database/
